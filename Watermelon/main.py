...
class Watermelon:
    def __init__(self):
        ...
        self.stripe1.color = engine_draw.darkgrey
        ...
        self.stripe2.color = engine_draw.darkgrey
...
class GroundSegment:
    def __init__(self, x, y, width, height, rotation=0, color=engine_draw.darkgrey):
        self.x = x
        self.physics = PhysicsRectangle2DNode()
        self.physics.position = Vector2(x, y)
        self.physics.width = width
        self.physics.height = height
        self.physics.dynamic = False
        self.physics.bounciness = 0.2
        self.physics.friction = 0.01  # Very low friction for smooth rolling
    ...
    def destroy(self):
        ...
        self.physics.position = Vector2(-9999, -9999)
...
class GroundManager:
    ...
    def generate_segment(self):
        """Generate next ground segment with random variations"""
        x = self.next_segment_x

        # Random height variation
        y_offset = (urandom.getrandbits(3) - 4)  # -4 to +3
        y = GROUND_Y_BASE + y_offset

        # Don't allow consecutive gaps
        if self.last_was_gap:
            segment_type = urandom.getrandbits(4) % 9  # 0-8, no gaps
        else:
            segment_type = urandom.getrandbits(4)  # 0-15

        self.last_was_gap = False

        if segment_type < 5:  # Flat ground (31%)
            segment = GroundSegment(x, y, GROUND_SEGMENT_WIDTH, GROUND_HEIGHT,
                                   color=engine_draw.darkgrey)
            self.next_segment_x = x + GROUND_SEGMENT_WIDTH - 2

        elif segment_type < 6:  # Upward ramp (6%)
            segment = GroundSegment(x, y - 3, GROUND_SEGMENT_WIDTH, GROUND_HEIGHT,
                                   rotation=-0.15, color=engine_draw.lightgrey)
            self.next_segment_x = x + GROUND_SEGMENT_WIDTH - 2

        elif segment_type < 7:  # Downward ramp (6%)
            segment = GroundSegment(x, y + 3, GROUND_SEGMENT_WIDTH, GROUND_HEIGHT,
                                   rotation=0.15, color=engine_draw.lightgrey)
            self.next_segment_x = x + GROUND_SEGMENT_WIDTH - 2

        elif segment_type < 8:  # Elevated platform (6%)
            segment = GroundSegment(x, y - 15, GROUND_SEGMENT_WIDTH * 0.7, GROUND_HEIGHT,
                                   color=engine_draw.white)
            self.next_segment_x = x + int(GROUND_SEGMENT_WIDTH * 0.7) - 2

        elif segment_type < 9:  # Dip/valley (6%)
            # Create a small lowered section
            segment = GroundSegment(x, y + BUMP_HEIGHT, GROUND_SEGMENT_WIDTH * 0.5,
                                   GROUND_HEIGHT, color=engine_draw.lightgrey)
            self.next_segment_x = x + int(GROUND_SEGMENT_WIDTH * 0.5) - 2

        else:  # Gap (44%)
            # No segment created, just advance position
            gap_width = GAP_WIDTH_MIN + (urandom.getrandbits(4) % (GAP_WIDTH_MAX - GAP_WIDTH_MIN))
            self.next_segment_x = x + gap_width
    ...
    def update(self, camera_x):
        """Generate new segments ahead and clean up old ones"""
        ...
        cleanup_x = camera_x - SCREEN_WIDTH * 1.5
        segments_to_remove = []
...
death_text.position = Vector2(-25, -20)
...
new_record_text.position = Vector2(-40, -35)
...
while True:
    if engine.tick():
        ...
        if state == STATE_TITLE:
            ...
            death_text.position.x = camera.position.x - 25
            ...
            new_record_text.position.x = camera.position.x - 40
